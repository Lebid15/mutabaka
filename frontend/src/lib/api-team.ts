...
export async function listTeam(token: string): Promise<TeamMember[]> {
  const res = await fetch(apiUrl('/api/team/'), { headers: { Authorization: `Bearer ${token}` } });
...
export async function updateTeamMember(token: string, id: number, payload: Partial<{ display_name: string; phone: string; password: string }>) {
...
export async function listConversationMembers(token: string, conversationId: number) {
  const res = await fetch(apiUrl(`/api/conversations/${conversationId}/members/`), { headers: { Authorization: `Bearer ${token}` } });
...
export async function addTeamMemberToConversation(token: string, conversationId: number, teamMemberId: number) {
  const res = await fetch(apiUrl(`/api/conversations/${conversationId}/add_team_member/`), {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
...
export async function removeMemberFromConversation(token: string, conversationId: number, memberId: number, memberType: 'user'|'team_member' = 'team_member') {
  const res = await fetch(apiUrl(`/api/conversations/${conversationId}/remove_member/`), {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
