          ...
          <div key={m.id} className={m.sender?.username === conv?.user_a?.username ? 'self-end text-left' : 'self-start text-right'}>
            <div className="inline-block bg-white/5 border border-white/10 rounded-2xl px-3 py-2">
              ...
              <div className="text-sm leading-relaxed">
                <span className="whitespace-pre-wrap break-words">{(m.body || '').replace(/\s+$/,'')}</span><span className="ml-2 text-[10px] text-gray-400 align-baseline whitespace-nowrap" dir="ltr">{new Date(m.created_at).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}</span>
              </div>
            </div>
          </div>
        ))}
